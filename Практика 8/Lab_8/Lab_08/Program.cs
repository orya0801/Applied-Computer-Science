...
namespace Lab_08
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Miln(x, y1, y2, f1, f2, 10, 0, 1, 0, 1);

            #endregion
        }

        private static void Miln(Expr x, Expr y1, Expr y2, 
            ...
            int divisions, double start_point, double end_point, double y1_start, double y2_start)
        {
            ...
            double h = (start_point - end_point) / divisions;
            ...
            for(int i = 0; i <= 3; i++)
            {
                ...
                dy1[i + 1] = dy1[i] + (k1 + 2 * k2 + 2 * k3 + k4) / 6;
                dy2[i + 1] = dy2[i] + (l1 + 2 * l2 + 2 * l3 + l4) / 6;

                x_val[i + 1] = x_val[i] + h;
            }

            double[] y1_pr, y1_kor, y2_pr, y2_kor;
            double dy1_1, dy2_1, dy1_2, dy2_2;

            y1_pr = y1_kor = y2_pr = y2_kor = new double[divisions + 1];
            ...
            for(int i = 3; i <= divisions; i++)
            {
                ...
                y1_pr[i + 1] = y1_val[i - 3] + (4 * h) / 3 * (2 * f1(x_val[i - 2], y1_val[i - 2], y2_val[i - 2]) - f1(x_val[i - 1], y1_val[i - 1], y2_val[i - 1]) + 2 * f1(x_val[i], y1_val[i], y2_val[i]));

                y2_pr[i + 1] = y2_val[i - 3] + (4 * h) / 3 * (2 * f2(x_val[i - 2], y1_val[i - 2], y2_val[i - 2]) - f2(x_val[i - 1], y1_val[i - 1], y2_val[i - 1]) + 2 * f2(x_val[i], y1_val[i], y2_val[i]));
              

                dy1_1 = f1(x_val[i + 1], y1_pr[i + 1], y2_pr[i + 1]);
                dy2_1 = f2(x_val[i + 1], y1_pr[i + 1], y2_pr[i + 1]);

                while (true)
                {
                    y1_kor[i + 1] = y1_val[i - 1] + (h / 3) * (dy1_1 + 4 * f1(x_val[i], y1_val[i], y2_val[i]) + f1(x_val[i - 1], y1_pr[i - 1], y2_val[i - 1]));
                    y2_kor[i + 1] = y2_val[i - 1] + (h / 3) * (dy2_1 + 4 * f2(x_val[i], y1_val[i], y2_val[i]) + f2(x_val[i - 1], y1_pr[i - 1], y2_val[i - 1]));
                    dy1_2 = f1(x_val[i + 1], y1_kor[i], y2_kor[i]);
                    dy2_2 = f1(x_val[i + 1], y1_kor[i], y2_kor[i]);
                    if (Math.Abs(dy2_2 - dy2_1) < 1e-3 && Math.Abs(dy1_2 - dy1_1) < 1e-3) break;
                    dy1_1 = dy1_2;
                    dy2_1 = dy2_2;
                }

                y1_val[i + 1] = y1_val[i - 2] + (h / 3) * (f1(x_val[i - 2], y1_val[i - 2], y2_val[i - 2]) + 4 * f1(x_val[i - 1], y1_val[i - 1], y2_val[i - 1]) + f1(x_val[i], y1_pr[i], y2_val[i]));
                y2_val[i + 1] = y2_val[i - 2] + (h / 3) * (f1(x_val[i - 2], y1_val[i - 2], y2_val[i - 2]) + 4 * f1(x_val[i - 1], y1_val[i - 1], y2_val[i - 1]) + f1(x_val[i], y1_pr[i], y2_val[i]));
            }
